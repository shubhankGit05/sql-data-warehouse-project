...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
			...
			CASE UPPER(TRIM(prd_line))     -- Quick form of CASE-WHEN for simple value mapping
		...
		CASE WHEN UPPER(TRIM(cntry)) IN ('US', 'USA', 'UNITED STATES') THEN 'United States'
			 WHEN UPPER(TRIM(cntry)) IN ('UK', 'GB', 'GBR', 'GREAT BRITAIN', 'UNITED KINGDOM') THEN 'United Kingdom'
